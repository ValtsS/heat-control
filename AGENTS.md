...
- **Thermostat vs DS1820 gotcha (do NOT misread):** the DS1820 temp (surface/external) reads ~5-7 °C LOWER than the boiler's **internal** thermostat (measures at a different location, near the element). When the tank is genuinely full, the internal thermostat opens, so `heatCmd=1` but `heatOn=0` (relay/`active_grid` confirm NO power reaches the element). This is **correct** behaviour, not a stuck relay or controller bug: the MPC idles in soak mode, never imports, and the "excess" free surplus is just exported. The soak's `T<maxTemp` check uses the DS1820, so it keeps commanding heat while the thermostat throttles — harmless. Do not "fix" by chasing `heatCmd!=heatOn` mismatches near the top of the tank.
...
- Local-hour resolution is **injectable** (`getLocalHour`, default `d.getHours()`); tests pass `(d)=>d.getUTCHours()` for determinism. Morning window uses local hours. **`TZ` must be set in `.env`** (IANA name, e.g. `Europe/Riga`) — the container defaults to UTC, so without it the morning window + schedule fire up to 3h late (Riga is UTC+2/+3). `dotenv.config()` runs before any request, so `process.env.TZ` is set before `Date.getHours()` is ever called.
...
- **DS1820 sensor noise gotcha:** the surface probe (on top of the tank) occasionally **spikes ±4-6 °C in a single 5 s poll** (relay/heater electrical noise on the 1-wire bus). A 150 L tank can't move 2 °C in 5 s, so these are fake. `src/tempFilter.ts` rejects jumps > `TEMP_FILTER_MAX_JUMP` (2 °C) and EWMA-smooths the rest (`TEMP_FILTER_BLEND` 0.3); the **filtered** temp drives the controller while the **raw** value is logged (`DecisionLog.tempRaw`, `/debug.tempFilter`). Hysteresis (`HYSTERESIS_DEG`) is applied as a temp deadband on the MPC decision via `applyHysteresis` (`control.ts`). On Sep 19 this noise caused 1849 >2 °C jumps, 3159 fake readings above the 63 °C thermostat cap, and ~5 kWh of spurious night imports.
