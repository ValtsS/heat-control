...
- Decision (`control.ts:GetStateWithForecast`): legionellaForced → **unconditionally** `enableHeater=true` (grid). Else `plan = planTank(...)`; `morning` window + `poor = bankDelta>0`. Rules in order: morning `T<MORNING_TEMP` → always (hard floor, may import); morning + poor + `T<MORNING_POOR_TEMP` → import to bare min; stale forecast → sun-gate `elev>=MinElevDeg && T<requiredNow`; poor + `bankable` → heat toward `requiredNow` (bank from real solar surplus); `solarToday >= MIN_SOLAR_TODAY_KWH` → heat toward `requiredNoBank` (use solar, don't export); else off (crap day bare-min only). `HEATER_WATTS`, `MORNING_*`, `MIN_SOLAR_TODAY_KWH`, `MinElevDeg` env-configurable. Boiler is 2.4 kW single-phase; `active_grid_B_power_W` is that phase's net surplus.
- `planTank` math: `EPD = litres*0.001161`; `solarToday/Tomorrow = ForecastProcessor.calcKWh(forecast, from, to, 0)`; `needTomorrow = (targetTemp-minTemp)*EPD + tankLossKwhPerDay + usageKwhPerDay`; `bankDelta = clamp((needTomorrow - solarTomorrow)/EPD, 0, maxBankDeg)` only if forecast fresh; `bankable = solarToday > (targetTemp-tempNow)*EPD + lossToEod` (real solar surplus to bank with); `targetEod = clamp(targetTemp+bankDelta, minTemp, TANK_MAX_TEMP≤65)`; `requiredNow = targetEod - (solarToday - lossToEod)/EPD`; `requiredNoBank = clamp(targetTemp - (solarToday - lossToEod)/EPD, minTemp, ceiling)`.
