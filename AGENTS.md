...
- Decision (`control.ts:GetStateWithForecast`): legionellaForced → **unconditionally** `enableHeater=true` (grid). Else `plan = planTank(...)`; `morning` window + `poor = plan.poor`. Rules in order: morning `T<MORNING_TEMP` → always (hard floor, may import); morning + poor + `T<MORNING_POOR_TEMP` → import to bare min; stale forecast → sun-gate `elev>=MinElevDeg && T<requiredNow`; poor + `bankable` → heat toward `requiredNow` (bank only when today's boiler-phase solar can fund the full bank); `solarToday >= MIN_SOLAR_TODAY_KWH` → heat toward `requiredNoBank` (use solar, don't export); else off (crap day bare-min only). `HEATER_WATTS`, `MORNING_*`, `MIN_SOLAR_TODAY_KWH`, `MinElevDeg`, `BOILER_PHASE_SHARE` env-configurable. Boiler is 2.4 kW single-phase; `active_grid_B_power_W` is that phase's net surplus.
- `planTank` math: `EPD = litres*0.001161`; total-array `solarToday/Tomorrow = calcKWh(...)` then scaled by `BOILER_PHASE_SHARE` (only the boiler's phase reaches the heater) → `solarToday/solarTomorrow` reported in **boiler-phase kWh**; `needTomorrow = (targetTemp-minTemp)*EPD + tankLossKwhPerDay + usageKwhPerDay`; `poor = shortfall = needTomorrow - boilerSolarTomorrow > 0` (tomorrow can't meet its own need, independent of today's surplus); `energyToTarget = max(0,(targetTemp-tempNow)*EPD + lossToEod)`; `bankDelta = min(shortfall/EPD, maxBankDeg)` **only if** `boilerSolarToday - energyToTarget >= bankDelta*EPD` — bank from today's real boiler-phase surplus, otherwise no bank at all (no night grid-import to bank); `targetEod = clamp(targetTemp+bankDelta, minTemp, TANK_MAX_TEMP≤65)`; `requiredNow = targetEod - (boilerSolarToday - lossToEod)/EPD`; `requiredNoBank = clamp(targetTemp - (boilerSolarToday - lossToEod)/EPD, minTemp, ceiling)`.
